...
class ExistingWarmupIntegration:
    ...
    def ensure_warmup_enabled(self) -> None:
        ...
        logger.info(f"Tiered loading enabled: {self.tiered_config.enable_tiered_loading}")
        ...
        logger.info(f"Warm popular models: {self.tiered_config.warm_popular_models_count}")
        ...
        if not self.tiered_config.enable_model_warming:
            logger.warning("Model warming is disabled - consider enabling for better performance")

        if self.tiered_config.hot_cache_size < 10:
            logger.warning("Hot cache size is small - consider increasing for better performance")
