...
class MLflowClient:
    ...
    def get_model_parameters_by_stage(self,
                                      model_name: str,
                                      stages: str = "Production") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.debug(f"Found Production model {model_name} version {version_number}")
            ...
            if hasattr(production_version, 'tags') and production_version.tags:
                ...
                self.logger.debug(f"Loaded {len(production_version.tags)} parameters from model version tags")
            ...
            try:
                ...
                if hasattr(run.data, 'params') and run.data.params:
                    ...
                    self.logger.debug(f"Added {len(run.data.params)} parameters from run params")
                ...
                if hasattr(run.data, 'tags') and run.data.tags:
                    ...
                    self.logger.debug(f"Added {len(run.data.tags)} parameters from run tags")
            ...
            try:
                ...
                if hasattr(registered_model, 'tags') and registered_model.tags:
                    ...
                    self.logger.debug(f"Added fallback parameters from registered model tags")

            except Exception as e:
                self.logger.warning(f"Could not fetch registered model tags for {model_name}: {e}")

            self.logger.debug(f"Loaded {len(parameters)} parameters for Production model {model_name}")
            return parameters

        except Exception as e:
            self.logger.error(f"Error getting Production model parameters for {model_name}: {e}")
            return {}
        ...
        for key in threshold_keys:
            if key in all_params:
                try:
                    ...
                    self.logger.warning(f"Could not convert {key}={all_params[key]} to float for model {model_name}")
