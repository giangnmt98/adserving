...
class RequestProcessor:
    ...
    def prepare_input_data(self, request: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        self._validate_prediction_tasks(prediction_tasks)
    ...
    def _create_prediction_task(
        ...
        input_features = {fn_field: gia_tri, "gia_tri": gia_tri}
    ...
    def _validate_prediction_tasks(
        self, prediction_tasks: List[Dict[str, Any]]
    ) -> None:
        if not prediction_tasks:
            self.logger.error(
                "No valid prediction tasks could be created from the request"
            )
            raise ValueError(
                "No valid prediction tasks could be created. "
                "Please check your request format and data."
            )

        self.logger.debug(
            f"Successfully created {len(prediction_tasks)} prediction tasks"
        )
