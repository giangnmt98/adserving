...
from adserving.core.utils.mlflow_parameter_updater import MLflowParameterUpdater
...
async def get_model_stats(manager: ModelManager = Depends(get_model_manager)):
    ...
    try:
        ...
        cache_hit_rate = performance.get("hit_rate", 0.0)
        avg_load_time = performance.get("avg_load_time", 0.0)
...
async def update_model_threshold(
    ...
    """Update anomaly threshold for a specific model"""
    try:
        parameter_updater = MLflowParameterUpdater(manager.mlflow_client)
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to update threshold"
            )
...
async def update_model_parameters(
    ...
    try:
        ...
        success = parameter_updater.create_parameter_version(
            ...
            comment=request.comment or f"API batch parameter update"
        )

        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to update parameters"
            )
...
async def rollback_model_version(
    ...
    try:
        ...
        success = parameter_updater.rollback_to_version(
            ...
            comment=request.comment or f"API rollback to version {request.target_version}"
        )

        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to rollback model"
            )
...
async def evict_model_from_cache(
    ...
    try:
        ...
        return {
            ...
            "model_name": model_name,
            "timestamp": datetime.now().isoformat(),
            "message": f"Model {'removed from cache' if removed else 'not found in cache'}"
        }
    except Exception as e:
        logger.error(f"Error evicting model {model_name}: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to evict model: {str(e)}"
        )
