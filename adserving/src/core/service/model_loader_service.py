    ...
    def load_model_sync(self, model_name: str) -> Optional[ModelInfo]:
        ...
        try:
            ...
            try:
                ...
                try:
                    anomaly_threshold = mlflow_client.get_threshold_parameters(
                        model_name
                    )
                except Exception as e:
                    self.logger.debug(
                        f"No threshold parameters found for {model_name}: {e}"
                    )
                    anomaly_threshold = {}

            except Exception as e:
                self.logger.error(
                    f"Error getting production version for model {model_name}: {e}"
                )
                return None

            if not latest_version:
                self.logger.error(
                    f"Could not find any suitable version for model {model_name}"
                )
                return None
