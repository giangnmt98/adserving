...
class MLflowParameterUpdater:
    ...
    def _create_run_with_updated_parameters(
        ...
        try:
            ...
            with mlflow.start_run(experiment_id=base_run.info.experiment_id) as new_run:
                ...
                mlflow.set_tag(
                    "parameter_update_comment", comment or "Parameter update"
                )
    ...
    def _create_model_placeholder(self, model_name: str, current_version: str) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"Created placeholder for {model_name} v{current_version}"
            )
            ...
            for param_key, param_value in parameter_updates.items():
                if param_key == "anomaly_threshold":
                    ...
                    if not isinstance(param_value, (int, float)):
                        ...
                        validation_result["warnings"].append(
                            f"contamination {param_value} " f"outside range (0.0, 0.5)"
                        )
                ...
                try:
                    ...
                    if tags.get("parameter_update") == "true":
                        ...
                        update_record = {
                            ...
                            "comment": tags.get(
                                "parameter_update_comment", "No comment"
                            ),
                    ...
                    self.logger.debug(
                        f"Error processing version {version.version}: {e}"
                    )
                    continue
    ...
    def get_model_version_parameters(
        ...
        try:
            ...
            self.logger.error(
                f"Error getting parameters for {model_name} v{version}: {e}"
            )
            return {}
